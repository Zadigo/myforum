        ...
        <div class="col-sm-12 col-md-8 offset-md-2">
          <div v-if="currentStage == 1" class="card">
            <div class="card-body">
              <form @submit.prevent>
                <div>
                  <input id="username" v-model="credentials.username" type="text" autocomplete="username" class="form-control p-3" placeholder="Username">
                ...
                <input v-model="credentials.email" type="email" autocomplete="email" class="form-control p-3 my-1" placeholder="Email address">
                <input v-model="credentials.password1" type="password" autocomplete="new-password" class="form-control p-3 my-1" placeholder="Password">
                <input v-model="credentials.password2" type="password" autocomplete="new-password" class="form-control p-3 my-1" placeholder="Enter password again">

                <div class="form-check form-switch mt-4">
                  <input v-model="termsAndPolicies" class="form-check-input" type="checkbox" role="switch" id="terms-and-policies">
                  <label class="form-check-label" for="terms-and-policies">I agree to terms and privacy policies</label>
                </div>

                <button :class="{ disabled: !canBeSubmitted }" type="button" class="btn btn-lg btn-primary mt-4" @click="register">
                  Register
                </button>
              </form>
            </div>
          ...
          <div v-if="currentStage == 2" class="card">
            <div class="card-body">
              ...
              <button type="button" class="btn btn-lg btn-primary mt-4" @click="completeRegistration">
...
export default {
  ...
  computed: {
    canBeSubmitted() {
      return this.termsAndPolicies || false
    }
  },
  beforeMount() {
    const registered = this.$session.retrieve('registered')
    if (registered) {
      this.currentStage = 2
    }
  },
